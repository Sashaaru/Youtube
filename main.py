...
with st.sidebar:
    add_radio=st.radio("youtube data harvesting and warehousing project",("Home","Extract & Transfer","queries"))
...
rain(
    ...
    font_size=28,
    falling_speed=10,
    animation_length="10SEC",
)

client=pymongo.MongoClient("mongodb://port:8501")
db=client.real
...
api_key="AIzaSyCsq_MlzsfC5KUVja45DUCcQRov8LzyBUo"
...
if add_radio == "Home":
        st.title(":red[Project title]: Youtube data harvesting and Warehousing project")
        st.title(":red[Domain] : Social Media")
        st.title(":red[Technologies used] : Python,MongoDB, Youtube Data API, MySql, Streamlit")
        st.title(":red[Overview] : Retrieving the Youtube channels data from the Google API, storing it in a MongoDB as data lake, migrating and transforming data into a SQL database,then querying the data and displaying it in the Streamlit app")
    # col2.markdown("#   ")
    # col2.markdown("#   ")
    # col2.markdown("#   ")
...
if add_radio == "Extract & Transfer":
    ...
    with tab1:
        ...
        ch_id = st.text_input(
            "Hint : Goto channel's home page > Right click > View page source > ctrl+F>?channel_id>copy channel_id").split(
            ',')
